...
class SequenceClassificationEvaluator:
    ...
    def get_metrics(self, verbose=False):
        cm = self.count_matrix_network_seq
        num_classes = 19
        ...
        for i in range(num_classes):
            label_name = self.name_labels[i]
            result[f"{label_name}_precision"] = precision[i]
            result[f"{label_name}_recall"] = recall[i]
            result[f"{label_name}_accuracy"] = accuracy[i]
            result[f"{label_name}_f1"] = f1[i]

        
        result["macro_precision"] = np.mean(precision)
        result["macro_recall"] = np.mean(recall)
        result["macro_accuracy"] = np.mean(accuracy)
        result["macro_f1"] = np.mean(f1)
